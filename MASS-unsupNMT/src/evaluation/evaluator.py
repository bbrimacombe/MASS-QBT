...
class Evaluator(object):
    ...
    def __init__(self, trainer, data, params):
        ...
        if self.params.is_master:
            ...
            params.hyp_path = os.path.abspath(params.hyp_path)
    ...
    def create_reference_files(self):
        ...
        for (lang1, lang2), v in self.data['para'].items():
            ...
            for data_set in ['valid', 'test']:
                ...
                lang1_path = os.path.abspath(lang1_path)
                lang2_path = os.path.abspath(lang2_path)
        ...
        with torch.no_grad():
            ...
            for data_set in ['valid', 'test']:
                ...
                for lang1, lang2 in set(params.mt_steps + [(l2, l3) for _, l2, l3 in params.bt_steps] + mass_steps \
                    + [(l2, l3) for _, l2, l3 in params.qbt_steps]):
                    eval_bleu = params.eval_bleu and params.is_master
                    self.evaluate_mt(scores, data_set, lang1, lang2, eval_bleu)

                for lang1, lang2 in set([(l2, l3) for _, l2, l3 in params.encoder_bt_steps]):
                    eval_bleu = params.eval_bleu and params.is_master
                    self.eval_encoder_trans(scores, data_set, lang1, lang2, eval_bleu)
...
class EncDecEvaluator(Evaluator):
    ...
    def eval_encoder_trans(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            # cuda
            x2, len2 = None, None  # THese are "cheating" here
            positions = None

            #len1 = torch.Tensor([len1.max()]*len(len1)) #####  #Old...
            len1 = torch.Tensor([x1.size()[0]]*len(len1))  # From bt trans
            x1, len1, langs1 = to_cuda(x1, len1, langs1)

            #pad_len = int(2 + random.randint(1, 4))
            #x1, len1, langs1 = self.encoder.pad_for_encoder_trans(x1, len1, langs1, pad_len)

            #enc_dict = {"x": x1, "lengths": len1, "langs":langs1, "pad_len": pad_len}
            enc_dict = {"x": x1, "lengths": len1, "langs":langs1}
            generated, _ = self.encoder.generate(src_enc=None, src_len=None, tgt_lang_id=None, enc_dict=enc_dict, sample_temperature=None)

            #generate translation - translate / convert to text
            if eval_bleu:
                idk = torch.Tensor([len1.max()]*len(len1)) #max-1?
                hypothesis.extend(convert_to_text(generated, idk, self.dico, params))
        ...
        if eval_bleu:
            ...
            # hypothesis / reference paths
            hyp_name = 'encoder-hyp{0}.{1}-{2}.{3}.txt'.format(scores['epoch'], lang1, lang2, data_set)
            hyp_path = os.path.join(params.hyp_path, hyp_name)
            ref_path = params.ref_paths[(lang1, lang2, data_set)]
...
def convert_to_text(batch, lengths, dico, params):
    ...
    lengths = lengths.cpu().numpy().astype(int)
    ...
    for j in range(bs):
        ...
        for k in range(1, lengths[j]):
            if batch[k, j] == params.eos_index:
                break
            #New
            if batch[k, j] == params.pad_index:  ## ! important for eval now
                break
            #if batch[k, j] == params.eos_index:
            #    words.append("!e!")
...
def eval_moses_bleu(ref, hyp):
    ...
    #BLEU_SCRIPT_PATH = BLEU_SCRIPT_PATH.replace("C:\\Users\\Ben\\Desktop\\MASS-Master\\MASS-master\\MASS-unsupNMT\\", "").replace("\\", "/")
    ...
    command = f'wsl {BLEU_SCRIPT_PATH}' + ' %s < %s'
