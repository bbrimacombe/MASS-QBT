...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        self.stats = OrderedDict(
            ...
            [('En-Rand-MT', [])] +
            [('En-BT-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.encoder_bt_steps] +
            [('QBT-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.qbt_steps]
        )
    ...
    def get_batch(self, iter_name, lang1, lang2=None, stream=False, double_mono=False, back=False):
        """
        ...
        ###################For noising
        if double_mono:
            iterator_1 = self.iterators.get((iter_name, lang2, None), None)
            iterator_2 = self.iterators.get((iter_name, lang1, None), None)
            if iterator_1 is None:
                iterator_1 = self.get_iterator(iter_name, lang2, None, stream, back)
            if iterator_2 is None:
                iterator_2 = self.get_iterator(iter_name, lang1, None, stream, back)
            try:
                x1, x2 = next(iterator_1), next(iterator_2)
    ...
    def encoder_bt_step(self, lang1, lang2, lang3, lambda_coeff):
        """
        ...
        ### Prob some memory optimization to be done with del the objects...
        ...
        try:
            ...
            tensor = self.encoder.fwd(x=x2, lengths=len2, langs=langs2, causal=False)
            scores, loss = self.encoder.predict(tensor=tensor, pred_mask=pred_mask, y=y, get_scores=True)
            self.stats[('En-BT-%s-%s-%s' % (lang1, lang2, lang3))].append(loss.item())
            loss = lambda_coeff * loss
    ...
    def encoder_no_repeat_step(self, lang1, lang2):
        ...
        try:
            ...
            x, lengths, positions, langs1, (_, _) = self.generate_batch(lang1=lang1, lang2=None, name='pred')
            #pad_len = int(3)
            #x, lengths, langs1 = self.encoder.pad_for_encoder_trans(x, lengths, langs1, pad_len)

            x, lengths, langs1 = to_cuda(x, lengths, langs1)

            alen = torch.arange(lengths.max(), dtype=torch.long, device=lengths.device)
            pred_mask = alen[:, None] < lengths[None] #- 1  # do not predict anything given the last target word
            y = x.masked_select(pred_mask)
    ...
    def encoder_mt_random(self, lang1, lang2, lambda_coeff):
        ...
        if True: #try:
            ...
            x2 = x2[:min(x2.size()[0], x1.size()[0]), :min(x2.size()[1], x1.size()[1])]
            len1 = torch.clip(len1, max=min(len1.max(), len2.max()))[:min(len1.size()[0], len2.size()[0])]
            len2 = torch.clip(len2, max=min(len1.max(), len2.max()))[:min(len1.size()[0], len2.size()[0])]
            ...
            ###################
            alen = torch.arange(len2.max(), dtype=torch.long, device=len2.device)
            pred_mask_1 = alen[:, None] < len2[None] - 1  # do not predict anything given the last target word
            y_1 = x2.masked_select(pred_mask_1)
            ...
            tensor = self.encoder('fwd', x=x2, lengths=len2, positions=None, langs=langs2, causal=False)
            _, loss_2 = self.encoder('predict', tensor=tensor, pred_mask=pred_mask_2, y=y_2, get_scores=False)

            loss = loss_1 + loss_2

            self.stats[('En-Rand-MT')].append(loss.item())
            ...
            # number of processed sentences / words
            self.n_sentences += params.batch_size
            self.stats['processed_s'] += len1.size(0) + len2.size(0) + 1
            self.stats['processed_w'] += pred_mask_1.sum().item() + pred_mask_2.sum().item() + 1
        #except Exception as e:
        #    print("\n\n MT Random Exception:", e, "\n\n")
        ######
        ######
        ######
...
class EncDecTrainer(Trainer):
    ...
    def qbt_step(self, lang1, lang2, lang3, lambda_coeff):
        """
        Back-translation step for machine translation.
        """
        self.cnt += 1
        ...
        # cuda
        x_noise, x, lengths, langs1 = to_cuda(x_noise, x, lengths, langs1)
        enc_dict = {"x": x_noise, "lengths": lengths, "langs":langs1}
        #logger.info("DICT: ", enc_dict)
        ...
        try:
            ...
            alen = torch.arange(lengths.max(), dtype=torch.long, device=lengths.device)
            pred_mask = alen[:, None] < lengths[None] - 1  # do not predict anything given the last target word
            y1 = x[1:].masked_select(pred_mask[:-1])

            # decode original sentence
            dec3 = self.decoder('fwd', x=x, lengths=lengths, langs=langs1, causal=True, src_enc=enc2, src_len=len2)

            del enc2

            # loss
            _, loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y1, get_scores=False)
            self.stats[('QBT-%s-%s-%s' % (lang1, lang2, lang3))].append(loss.item())
            loss = lambda_coeff * loss
